...
{
public:
    double pow, eps, etrue_cart, etrue_per;
    std::vector<double> x, g, gnum, radii, boxv;
    virtual void SetUp(){
    	pow = 2.5;
    	eps = 1;
        ...
        radii[2] = 1.13 * f;
        boxv = {1., 1., 1.};
        etrue_cart = 0.023173380132354017;
        etrue_per = 0.60660574285698954;
        g = std::vector<double>(x.size());
        gnum = std::vector<double>(x.size());
    }
};

TEST_F(InversePowerTest, Energy_Works_Cartesian){
    InversePowerCartesian<3> pot(pow, eps, radii);
    double e = pot.get_energy(x);
    ASSERT_NEAR(e, etrue_cart, 1e-10);
}

TEST_F(InversePowerTest, EnergyGradient_AgreesWithNumerical_Cartesian){
    InversePowerCartesian<3> pot(pow, eps, radii);
...
TEST_F(InversePowerTest, Energy_Works_Periodic){
    InversePowerPeriodic<3> pot(pow, eps, radii, boxv);
    double e = pot.get_energy(x);
    ASSERT_NEAR(e, etrue_per, 1e-10);
}

TEST_F(InversePowerTest, EnergyGradient_AgreesWithNumerical_Periodic){
    InversePowerPeriodic<3> pot(pow, eps, radii, boxv);
	double e = pot.get_energy_gradient(x, g);
	std::cout<<"energy"<<e<<std::endl;
    double ecomp = pot.get_energy(x);
...
TEST_F(InversePowerTest, EnergyGradientHessian_AgreesWithNumerical_Cartesian){
	InversePowerCartesian<3> pot(pow, eps, radii);
...
TEST_F(InversePowerTest, EnergyGradientHessian_AgreesWithNumerical_Periodic){
	InversePowerPeriodic<3> pot(pow, eps, radii, boxv);
    std::vector<double> h(x.size()*x.size());
    std::vector<double> hnum(h.size());
    double e = pot.get_energy_gradient_hessian(x, g, h);
...
//     const int pow = 4;
//     pele::InversePowerPotential<3> pot(pow, eps, radii);
...
//     pot.numerical_gradient(x, gnum, 1e-6);
//     pele::std::vector<double> gnum_int(gnum.size());
//     pot_int.numerical_gradient(x, gnum_int, 1e-6);
//     for (size_t k = 0; k < 6; ++k) {
//         ASSERT_NEAR(gnum[k], gnum_int[k], 1e-6);
//     }
//     pele::std::vector<double> h(x.size()*x.size());
//     pele::std::vector<double> hnum(h.size());
//     pot.get_energy_gradient_hessian(x, g, h);
//     pele::std::vectorg_int(g.size());
//     pele::std::vectorh_int(h.size());
//     pot_int.get_energy_gradient_hessian(x, g_int, h_int);
//     pot.numerical_gradient(x, gnum);
//     pot.numerical_hessian(x, hnum);
//     pele::std::vector<double> hnum_int(hnum.size());
...
//                    0.179587122125166561689082,0.1013211836423377714438795,tol716435640373628375718308,

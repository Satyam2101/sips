#ifndef _HYPERLAG_CHECK_OVERLAP_CELL_LISTS_H
#define _HYPERLAG_CHECK_OVERLAP_CELL_LISTS_H
...
protected:
    ...
    const std::vector<double> m_radii;
    std::shared_ptr<ha::CellLists<DIST_POL> > m_cell_lists;
...
public:
    ...
    std::shared_ptr<ha::CellLists<DIST_POL> > cell_lists)
        :   m_dist(dist),
            m_radii(radii),
...
class CheckOverlapPeriodicCellLists : public CellListCheckOverlap<ha::periodic_distance<ndim> > {
public:
    CheckOverlapPeriodicCellLists(const std::vector<double> & radii, const std::vector<double> & boxvec, double ncellx_scale=1.0)
        : CellListCheckOverlap<ha::periodic_distance<ndim> >(radii,
            std::make_shared<ha::periodic_distance<ndim> >(boxvec),
            std::make_shared<ha::CellLists<ha::periodic_distance<ndim> > >(std::make_shared<ha::periodic_distance<ndim> >(boxvec), boxvec, 2 * (*std::max_element(radii.begin(), radii.end())), ncellx_scale))
    {}
};

template<size_t ndim>
class CheckOverlapCartesianCellLists : public CellListCheckOverlap<ha::cartesian_distance<ndim> > {
public:
    CheckOverlapCartesianCellLists(const std::vector<double> & radii, const std::vector<double> & boxvec, double ncellx_scale=1.0)
        : CellListCheckOverlap<ha::cartesian_distance<ndim> >(radii,
                std::make_shared<ha::cartesian_distance<ndim> >(),
                std::make_shared<ha::CellLists<ha::cartesian_distance<ndim> > >(std::make_shared<ha::cartesian_distance<ndim> >(), boxvec, ncellx_scale))
    {}
};

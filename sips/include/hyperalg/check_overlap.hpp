#ifndef _HYPERALG_CHECK_OVERLAP_HPP
#define _HYPERALG_CHECK_OVERLAP_HPP
...
class CheckOverlapPeriodic : public CheckOverlap<ha::periodic_distance<ndim> > {
public:
    CheckOverlapPeriodic(const std::vector<double> & radii, const std::vector<double> & boxvec)
        : CheckOverlap< ha::periodic_distance<ndim> >(radii,
                std::make_shared<ha::periodic_distance<ndim> >(boxvec))
    {}
};

template <size_t ndim>
class CheckOverlapCartesian : public CheckOverlap<ha::cartesian_distance<ndim> > {
public:
    CheckOverlapCartesian(const std::vector<double> & radii)
        : CheckOverlap<ha::cartesian_distance<ndim> >(radii,

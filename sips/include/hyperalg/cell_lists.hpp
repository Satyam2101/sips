#ifndef _HYPERALG_CELL_LISTS_HPP
#define _HYPERALG_CELL_LISTS_HPP
...
#pragma omp parallel
    ...
    public:
        ...
        std::vector<std::vector<cell_t *>> m_cell_neighbors;
...
#pragma omp parallel
...
#else
            const size_t nsubdoms = m_cell_atoms.size();
            for (size_t isubdom = 0; isubdom < nsubdoms; ++isubdom)
            {
                for (auto &v : m_cell_atoms[isubdom])
                {
         ...
         */
        inline void remove_atom_from_cell_ind(const size_t index, const size_t icell, const size_t isubdom)
        {
        ...
        {
            auto atom_ptr = std::find(m_cell_atoms[isubdom][icell].begin(), m_cell_atoms[isubdom][icell].end(), iatom);
            std::swap(*atom_ptr, m_cell_atoms[isubdom][icell].back());
            m_cell_atoms[isubdom][icell].pop_back();
        }
        /*
         * print all the atoms in the container
         */
        void print_container() const
        {
            std::cout << "PIRNTER CONTAINER STARTS" << std::endl;
            for (size_t isubdom = 0; isubdom < m_cell_atoms.size(); isubdom++)
            {
                for (size_t icell = 0; icell < m_cell_atoms[isubdom].size(); icell++)
                {
                    for (size_t iatom = 0; iatom < m_cell_atoms[isubdom][icell].size(); iatom++)
                    {
                        std::cout << "(" << isubdom << "," << icell << "," << m_cell_atoms[isubdom][icell][iatom] << ")  ";
                    }
                    std::cout << std::endl;
                }
            }
        ...
        void print_container(size_t isubdom, size_t icell) const
        {
            for (size_t iatom = 0; iatom < m_cell_atoms[isubdom][icell].size(); iatom++)
            {
                std::cout << "(" << isubdom << "," << icell << "," << m_cell_atoms[isubdom][icell][iatom] << ")  ";
...
namespace ha
{
    ...
    public:
        ...
        ha::VecN<ndim> m_boxvec;
        ha::VecN<ndim> m_inv_boxvec; //!< inverse of boxvec
        double m_rcut;
        cell_vec_t m_ncells_vec;    //!< the number of cells in each dimension
        ha::VecN<ndim> m_rcell_vec; //!< the cell length in each dimension
        size_t m_ncells;
        size_t m_nsubdoms;
        ...
        LatticeNeighbors(std::shared_ptr<distance_policy> const &dist,
                         ...
                         const double rcut,
                         std::vector<size_t> const &ncells_vec)
            : m_dist(dist),
              m_boxvec(boxvec),
              m_inv_boxvec(ndim),
              m_rcut(rcut),
        ...
        cell_vec_t position_to_cell_vec(const double *const x) const
        {
            ...
            for (size_t idim = 0; idim < ndim; ++idim)
            {
                cell_vec[idim] = m_ncells_vec[idim] * (x_in_box[idim] * m_inv_boxvec[idim] + 0.5 - std::numeric_limits<double>::epsilon());
        ...
        size_t get_subdomain(cell_vec_t const &v) const
        {
            ...
            {
                return std::upper_bound(m_subdom_limits.begin(), m_subdom_limits.end(), v[1]) - m_subdom_limits.begin() - 1;
            }
        }
        ...
        double minimum_distance(cell_vec_t const &v1, cell_vec_t const &v2) const
        {
            ...
            ha::VecN<ndim> ll1, ll2, dr;
            ha::VecN<ndim> minimum_dist; // the minimum possible distance in each direction
        ...
        void find_global_neighbor_inds(const size_t idim, cell_vec_t const &v0,
                                       ...
                                       cell_vec_t const &vorigin) const
        {
            if (idim == ndim)
            {
        ...
        void find_neighbor_pairs(
            ...
            std::vector<std::vector<cell_t *>> &cell_neighbors,
...
#pragma omp parallel
        ...
        void find_neighbor_pairs_subdom(
            std::vector<std::array<cell_t *, 2>> &neighbor_pairs_inner,
            std::vector<std::array<cell_t *, 2>> &neighbor_pairs_boundary,
            std::vector<std::vector<cell_t *>> &cell_neighbors,
            std::vector<std::vector<cell_t>> &cells,
            const size_t isubdom) const
        {
            neighbor_pairs_inner = std::vector<std::array<cell_t *, 2>>();
            neighbor_pairs_boundary = std::vector<std::array<cell_t *, 2>>();
            ...
            for (size_t local_icell = 0; local_icell < m_subdom_ncells[isubdom]; ++local_icell)
            {
                ...
                for (const size_t global_jcell : global_neighbors)
                {
                    ...
                    {
                        if (pos_direction_y(global_icell, global_jcell))
                        { // avoid duplicates, balance load
                            std::array<cell_t *, 2> neighbors = {&cells[isubdom][local_icell],
                                                                 &cells[jsubdom][local_jcell]};
            ...
            for (size_t i = 0; i < icells.size(); ++i)
            {
                ...
                for (cell_t *jcell : m_container.m_cell_neighbors[icells[i]])
                {
                    for (auto jatom = jcell->begin(); jatom != jcell->end(); ++jatom)
                    {
                        if (iatoms[i] != *jatom)
                        {
                            m_visitor.insert_atom_pair(iatoms[i], *jatom, isubdom);
                        }
                    }
                }
            }
    ...
    public:
        CellListsLoop(
            ...
            ha::LatticeNeighbors<distance_policy> lattice_tool)
            : m_visitor(visitor),
...
#pragma omp barrier
    ...
    protected:
        ...
        ha::LatticeNeighbors<distance_policy> m_lattice_tool;
    ...
    public:
        ...
        std::vector<size_t> get_order(const size_t natoms) const
        {
            auto order = std::vector<size_t>(natoms);
         ...
         */
        void update(std::vector<double> const &coords);
        void update_specific(std::vector<double> const &coords, std::vector<long> const &iatoms, std::vector<double> const &old_coords);
        inline bool is_initialized()
        {
            return m_initialized;
        }

        inline void print_container()
        {
            m_container.print_container();
        }

    protected:
        void print_warnings(const size_t natoms);
        void build_cell_neighbors_list();
        void reset_container(std::vector<double> const &coords);
        void update_container(std::vector<double> const &coords);
        void update_container_specific(std::vector<double> const &coords, std::vector<long> const &iatoms, std::vector<double> const &old_coords);

    private:
        static std::vector<size_t> get_ncells_vec(std::vector<double> const &boxv, const double rcut,
    ...
    template <typename distance_policy>
    std::vector<size_t> CellLists<distance_policy>::get_ncells_vec(std::vector<double> const &boxv, const double rcut, const double ncellx_scale, const bool balance_omp)
    {
        std::vector<size_t> res(boxv.size());
     ...
     */
    template <typename distance_policy>
    void CellLists<distance_policy>::update(std::vector<double> const &coords)
    {
        if (m_initialized)
        {
     ...
     */
    template <typename distance_policy>
    void CellLists<distance_policy>::update_specific(std::vector<double> const &coords, std::vector<long> const &iatoms, std::vector<double> const &old_coords)
    {
        if (m_initialized)
        {
     ...
     */
    template <typename distance_policy>
    void CellLists<distance_policy>::reset_container(std::vector<double> const &coords)
    {
        m_container.reset();
        const size_t natoms = coords.size() / m_ndim;
     ...
     */
    template <typename distance_policy>
    void CellLists<distance_policy>::update_container(std::vector<double> const &coords)
    {
#ifdef _OPENMP
#pragma omp parallel
        {
            ...
            for (size_t icell = 0; icell < m_lattice_tool.m_subdom_ncells[isubdom]; ++icell)
            {
                ...
                while (atom_nr < m_container.m_cell_atoms[isubdom][icell].size())
                {
                    ...
                    {
                        m_container.remove_atom_from_cell_ind(atom_nr, icell, isubdom);
...
#pragma omp barrier
     ...
     */
    template <typename distance_policy>
    void CellLists<distance_policy>::update_container_specific(std::vector<double> const &coords, std::vector<long> const &iatoms, std::vector<double> const &old_coords)
    {
        // coords is a size natoms*ndim vector,old_coords is size iatoms.size()*ndim vector for the old coords of iatoms
...
#endif // #ifndef _HYPERALG_CELL_LISTS_HPP
